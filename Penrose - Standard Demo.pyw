from turtle import lt, fd, rt, pos, clear, setup, setheading, heading, setpos, shapesize, shape, color, stamp
from turtle import tracer, begin_poly, end_poly, register_shape, get_poly, ht, pu, reset, goto, title, mode
from turtle import resizemode, bgcolor, pencolor, write, exitonclick
...
def test(length=200, n=4, fun=sun, start_pos=(0, 0), th=2):
    ...
    print("%d змеев and %d дротиков = %d кусочков." % (nk, nd, nk + nd))
...
def main():
    title('Мозаики Penrose с воздушными змеями и дротиками')
    mode("logo")
    bgcolor(0.3, 0.3, 0)
    demo(sun)
    sleep(2)
